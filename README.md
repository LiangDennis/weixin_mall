...
3. 点击收藏按钮
   1. 判断该商品是否存在与缓存中
   2. 已经存在 删除该商品
   3. 没有存在 把商品添加到收藏的数组中，并存入缓存中
4. 这时，之前保存的goodsInfo 就能更好的获取数据，因为goodsObj 只是渲染页面的时候需要展示的，而且属性不齐全，原因是我们只保存需要在页面中使用的属性，全部的属性都保存在goodsInfo中。
5. 当前页面是否选中isCollect与商品id进行关联
6. 个人中心页面收藏数量的添加
7. 是否登录才能收藏
   1. 应该是的，如果一部手机有两个用户分别登录，collect就不是很合适了

#### text标签中的enter键分行也会有影响

#### 搜索中心
1. 输入框绑定值变化事件 input事件
   1. 获取到输入框的值
   2. 合法性判断
   3. 检验通过 把输入框的值发送到后台
   4. 返回的数据打印到页面上
